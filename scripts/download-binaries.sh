...
LATEST_FILENAME=$(curl -s https://binaries.soliditylang.org/wasm/list.json | jq -r ".releases[\"$LATEST_VERSION\"]")
...
curl -f -L -o "embedded-binaries/$LATEST_FILENAME" "https://binaries.soliditylang.org/wasm/$LATEST_FILENAME"
...
V0821_FILENAME=$(curl -s https://binaries.soliditylang.org/wasm/list.json | jq -r '.releases["0.8.21"]')
echo "📥 Downloading LTS version: $V0821_FILENAME..."
curl -f -L -o "embedded-binaries/$V0821_FILENAME" "https://binaries.soliditylang.org/wasm/$V0821_FILENAME"
